...
email_env = Environment(loader=loader, autoescape=True)
...
class EmailTemplate:
    ...
    async def send_email(cls, email_to: str | list[str], context: dict, subject_override: str | None = None) -> None:
        try:
            ...
            logger.info(f"Email task with the task ID queued before {recipients_list}")
            
        except Exception as e:
            logger.error(f"Failed to queue email task for {recipients_list} {str(e)}")
            raise